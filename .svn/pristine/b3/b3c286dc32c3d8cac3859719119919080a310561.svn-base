...
const { getBulletDetail } = require('../../../apis/bullet.js');
const app = getApp()
app.Page({
  /**
   * 页面的初始数据
  ...
  // 获取弹幕
  getBulletDetailFn: function(spuid) {
    getBulletDetail(spuid).then(res => {
      console.log(res);
      this.setData({
        bullet: res
      })
  ...
  showData() {
    return getGoodsDetail({
      ...
      if (d.issoldout == 0 && d.status != 9) {
        this.getBulletDetailFn(this.data.spuid);
      }
      this.setData({
