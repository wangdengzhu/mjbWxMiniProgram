...
app.Page({
  ...
  formSubmit: function (e) {
    ...
    if (that.editContent) {
      if (!data.defaultflag && that.editContent.default_flag != 1) {
        defaultSet = 0;
      }
    } else {
      if (!data.defaultflag) {
    ...
    if (that.afterAddress) {
      ...
      if (afterAdd.street1.indexOf('暂无可选街道') > -1) {
        afterAdd.street1 = "";
      }
      afterAdd.street2 = data.addressdetail;
      console.log(afterAdd.street1)
    ...
    let sendAddress = {
      ...
      areaDName: that.street.indexOf('暂无可选街道') > -1 ? "" : that.street,
      areaaid: 0,
      areabid: 0,
      areacid: 0,
      areadid: 0,
