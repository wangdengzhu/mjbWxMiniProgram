...
const { checkPay, prePay, getDetail } = require('../../../apis/order.js');
const app = getApp()
app.Page({
  ...
  onLoad: function (options) {
    if (options.orderNo)
    {
      ...
      this.data.payStatus = Number(options.payStatus);
      this.showData();
    }
  },
  showData(){
    getDetail(this.data.orderNo).then(r=>{
        this.setData({
  ...
  {
    //orderno
    this.mjd.redirectTo(`/pages/order/list/list?status=0`)
  },
  onRePay: app.mjd.preventRepeat(function (submitComplete, e) {
    let { orderNo } = this.data, responseStatusCode = this.mjd.responseStatusCode;
    // 创建订单
